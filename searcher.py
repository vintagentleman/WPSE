...
class Searcher(object):
    ...
    def get_n_sort(self, query, doc, cit_off_lim):
        """
        Для данного документа (ибо вызывается в цикле покументам) сортирует позиции разных терминов запроса
        по возрастанию и выдаёт их в количестве, не превосходящем лимит (и пропускает n позиций, где n - оффсет).
        Сортировка производится без хранения массивов позиций для каждого термина в памяти.

        :param query: поисковый запрос
        :param doc: имя данного документа
        :param cit_off_lim: кортеж (один) вида (оффсет по цитатам, лимит по цитатам)
        :return: генератор отсортированных позиций
        """
    ...
    def search(self, query, doc_off, doc_lim, cit_off_lim):
        """
        Токенизирует поисковый запрос и ищет позиции его терминов в поисковом индексе.
        Все параметры 'протаскиваются' из frontend-функции построения предложений.

        :return: упорядоченный словарь вида {документ_1: [позиция_1a, позиция_1b], ...}
        """
    ...
    def tag(self, query, doc_off, doc_lim, cit_off_lim):
        """
        Вычленяет из текста предложения, в которых содержатся термины поискового запроса.
        Термины заключает в тег <b></b>; предложения, в которых содержится более одного термина,
        последовательно 'склеивает' в одно, с тем чтобы в выдаче оно отображалось единожды.

        :param query: поисковый запрос
        :param doc_off: оффсет по документам
        :param doc_lim: лимит по документам
        :param cit_off_lim: список пар вида (оффсет по цитатам, лимит по цитатам)
        :return: упорядоченный словарь вида {документ_1: [цитата_1a, цитата_1b], ...}
        """
